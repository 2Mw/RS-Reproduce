import copy
import os.path
from cf.config.dcn_me import config
...
def train(cfg, dataset: str = 'criteo', weights: str = ''):
    ...
    earlyStop = EarlyStopping('val_BCE', min_delta=0.0001, patience=3)
    aucStop = AbnormalAUC(0.83, steps=steps // 2, directory=directory, gap_steps=steps // 10)
